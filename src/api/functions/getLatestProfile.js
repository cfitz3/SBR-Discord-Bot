...
async function getLatestProfile(uuid, options = { museum: false }) {
  ...
  const [{ data: playerRes }, { data: profileRes }] = await Promise.all([
    axios.get(`https://api.hypixel.net/player?key=${config.api.hypixel_api_token}&uuid=${uuid}`),
    axios.get(`https://api.hypixel.net/skyblock/profiles?key=${config.api.hypixel_api_token}&uuid=${uuid}`),
  ]).catch((error) => {
